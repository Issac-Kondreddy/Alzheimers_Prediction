from Grasshopper_Optimization_Algorithm import GrasshopperAlgorithm  # Assuming you have saved GrasshopperAlgorithm class in GrasshopperAlgorithm.py
from Benchmark_functions import sphere, ackley, rastrigin, rosenbrock
import numpy as np
import matplotlib.pyplot as plt
import json

# Parameters for the Grasshopper Algorithm
population_size = 100
...
def run_grasshopper_on_function(function_name, function):
    print(f"Running Grasshopper algorithm on {function_name} function...")
    grasshopper_algo = GrasshopperAlgorithm(
    ...
    # Run the Grasshopper algorithm
    results = grasshopper_algo.run()
...
# Running the Grasshopper algorithm on each benchmark function and storing the results
for name, function in benchmark_functions.items():
    try:
        results[name] = run_grasshopper_on_function(name, function)
    except Exception as e:
        print(f"An error occurred while running the Grasshopper algorithm on the {name} function: {e}")

# Optional: Save the results to a JSON file for later analysis
with open('grasshopper_performance_results.json', 'w') as fp:
