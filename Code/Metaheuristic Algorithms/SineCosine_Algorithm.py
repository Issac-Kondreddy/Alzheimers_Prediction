...
class SineCosineAlgorithm:
    def __init__(self, objective_func, population_size, dimension, generations, a=3, r=1):
        ...
        self.a = a  # parameter 'a' for sine
        self.r = r  # parameter 'r' for cosine
    ...
    def move_particles(self):
        ...
        for i in range(self.population_size):
            for j in range(self.dimension):
                ...
                rand_sin = np.random.random()

                updated_population[i, j] = self.population[i, j] + (rand_1 * np.sin(rand_2) * (self.a ** rand_sin))

                if rand_1 < 0.5:
                    updated_population[i, j] += (rand_1 * np.cos(rand_2) * (self.r ** rand_sin))
                else:
                    updated_population[i, j] -= (rand_1 * np.cos(rand_2) * (self.r ** rand_sin))

                # Clamp values to the range [-5, 5]
                updated_population[i, j] = np.clip(updated_population[i, j], -5, 5)
