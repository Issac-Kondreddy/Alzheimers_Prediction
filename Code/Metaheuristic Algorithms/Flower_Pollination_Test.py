from Flower_Pollination import FlowerPollinationAlgorithm  # Assuming you have saved FlowerPollinationAlgorithm class in FlowerPollination.py
...
def run_flower_pollination_on_function(function_name, function):
    print(f"Running Flower Pollination algorithm on {function_name} function...")
    flower_algo = FlowerPollinationAlgorithm(
...
# Running the Flower Pollination algorithm on each benchmark function and storing the results
for name, function in benchmark_functions.items():
    try:
        results[name] = run_flower_pollination_on_function(name, function)
    except Exception as e:
        print(f"An error occurred while running the Flower Pollination algorithm on the {name} function: {e}")

# Optional: Save the results to a JSON file for later analysis
with open('flower_pollination_performance_results.json', 'w') as fp:
